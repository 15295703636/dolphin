...
public class CourseDeviceController {
    ...
    public ReturnInfo connect(@RequestBody Integer id) {
        return iCourseDeviceService.connect(id);
    }

    @PostMapping("hangUp")
    @ApiOperation(value = "挂断", notes = "日程-端管理")
    public ReturnInfo hangUp(@RequestBody Integer id) {
        return iCourseDeviceService.hangUp(id);
    }
